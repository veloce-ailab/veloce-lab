...
export default function ChatPage({ colors, agent, onBack }: Props) {
  ...
  return <>
    ...
    <Modal visible={settingsOpen} transparent animationType="slide" onRequestClose={() => setSettingsOpen(false)}><Sheet colors={colors} title="会话设置" onClose={() => setSettingsOpen(false)}><ScrollView contentContainerStyle={styles.settingsForm}><Choice colors={colors} label="模型" values={["", ...models]} labels={["使用代理默认模型", ...models]} value={config.model_name} onChange={(value: string) => setConfig(current => ({ ...current, model_name: value }))} /><Input colors={colors} label="最大 Token（0 为默认）" value={config.max_tokens} keyboardType="numeric" onChangeText={(value: string) => setConfig(current => ({ ...current, max_tokens: value }))} /><Input colors={colors} label="温度（留空为默认）" value={config.temperature} keyboardType="decimal-pad" onChangeText={(value: string) => setConfig(current => ({ ...current, temperature: value }))} /><Choice colors={colors} label="推理强度" values={["", "minimal", "low", "medium", "high"]} labels={["默认", "最小", "低", "中", "高"]} value={config.reasoning_effort} onChange={(value: string) => setConfig(current => ({ ...current, reasoning_effort: value }))} /><Toggle colors={colors} label="自动压缩上下文" value={config.auto_compress_context} onChange={(value: boolean) => setConfig(current => ({ ...current, auto_compress_context: value }))} /><Text style={{ color: colors.muted, fontSize: 12 }}>{activeID ? "保存后会应用于当前会话的后续消息。" : "新建会话的设置会在发送第一条消息时生效。"}</Text><Action colors={colors} title={activeID ? "保存会话设置" : "完成"} onPress={saveSessionSettings} /></ScrollView></Sheet></Modal>
  </>
}
