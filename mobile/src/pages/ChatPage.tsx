...
type SessionConfig = { model_name: string; user_channel_id: string; max_tokens: string; temperature: string; reasoning_effort: string; auto_compress_context: boolean; connector_device_id: string; connector_workspace_path: string; connector_approval_mode: "manual" | "safe" | "full_access"; disabled_tool_groups: string[] }
type Props = { colors: Palette; agent: Agent; onBack: () => void }

const defaultConfig = (agent: Agent): SessionConfig => ({ model_name: agent.default_model || "", user_channel_id: "0", max_tokens: "0", temperature: "", reasoning_effort: "", auto_compress_context: true, connector_device_id: "", connector_workspace_path: "", connector_approval_mode: "manual", disabled_tool_groups: [] })
const toolGroups = [{ key: "workspace", label: "工作区" }, { key: "web", label: "网页访问" }, { key: "tasks", label: "计划任务" }, { key: "ask_user", label: "主动提问" }, { key: "memory", label: "记忆" }]
...
export default function ChatPage({ colors, agent, onBack }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [sessionData, nextCatalog, nextDevices] = await Promise.all([request<Session[]>("/user/advanced-chat/sessions"), request<any[]>("/user/catalog"), request<any[]>("/user/advanced-chat/devices")])
      ...
      setModels([...new Set((Array.isArray(nextCatalog) ? nextCatalog : []).flatMap(item => item.models || []))])
  ...
  useEffect(() => { const subscription = BackHandler.addEventListener("hardwareBackPress", () => { if (devicePickerOpen) { setDevicePickerOpen(false); return true } if (settingsOpen) { setSettingsOpen(false); return true } if (menuOpen) { setMenuOpen(false); return true } if (drawer) { setDrawer(false); return true } onBack(); return true }); return () => subscription.remove() }, [devicePickerOpen, drawer, menuOpen, onBack, settingsOpen])

  const active = sessions.find(item => item.id === activeID)
  const channelModels = config.user_channel_id !== "0" ? (catalog.find(item => String(item.id) === config.user_channel_id)?.models || []) : models
  ...
  const send = async () => {
    ...
    try {
      await streamRequest("/user/advanced-chat/completions", {
        ...
        channel_id: Number(config.user_channel_id) || 0,
        mode: "assistant",
  ...
  return <>
    <View style={styles.flex}>
      ...
      {menuOpen ? <View style={[styles.actionMenu, { borderColor: colors.border, backgroundColor: colors.card }]}><Pressable onPress={() => { setMenuOpen(false); void chooseFiles() }} style={({ pressed }) => [styles.menuAction, pressed && styles.pressed]}><Ionicons name="attach-outline" size={20} color={colors.text} /><Text style={{ color: colors.text }}>上传附件</Text></Pressable><Pressable onPress={() => { setMenuOpen(false); setDevicePickerOpen(true) }} style={({ pressed }) => [styles.menuAction, pressed && styles.pressed]}><Ionicons name="laptop-outline" size={20} color={colors.text} /><Text style={{ color: colors.text }}>{config.connector_device_id ? devices.find(item => item.id === config.connector_device_id)?.name || "已选设备" : "选择设备"}</Text></Pressable><Pressable onPress={() => { setMenuOpen(false); setSettingsOpen(true) }} style={({ pressed }) => [styles.menuAction, pressed && styles.pressed]}><Ionicons name="settings-outline" size={20} color={colors.text} /><Text style={{ color: colors.text }}>会话设置</Text></Pressable></View> : null}<View style={[styles.composer, { borderColor: colors.border, backgroundColor: colors.card }]}><Pressable onPress={() => setMenuOpen(value => !value)} disabled={sending} hitSlop={6}><Ionicons name={menuOpen ? "close" : "add"} size={26} color={colors.text} /></Pressable><TextInput value={draft} onChangeText={setDraft} editable={!sending} multiline placeholder={`给${agent.name}发送消息`} placeholderTextColor={colors.muted} style={[styles.input, { color: colors.text, backgroundColor: colors.input }]} /><Pressable onPress={() => sending ? abortRef.current?.abort() : void send()}><Ionicons name={sending ? "stop" : "arrow-up"} size={21} color={sending ? colors.destructive : colors.text} /></Pressable></View>
    </View>
    ...
    <Modal visible={settingsOpen} transparent animationType="slide" onRequestClose={() => setSettingsOpen(false)}><Sheet colors={colors} title="会话设置" onClose={() => setSettingsOpen(false)}><ScrollView contentContainerStyle={styles.settingsForm}><Choice colors={colors} label="上游渠道" values={["0", ...catalog.map(item => String(item.id))]} labels={["使用默认渠道", ...catalog.map(item => item.name)]} value={config.user_channel_id} onChange={(value: string) => setConfig(current => ({ ...current, user_channel_id: value, model_name: "" }))} /><Choice colors={colors} label="模型" values={["", ...channelModels]} labels={["使用代理默认模型", ...channelModels]} value={config.model_name} onChange={(value: string) => setConfig(current => ({ ...current, model_name: value }))} /><Choice colors={colors} label="连接器设备" values={["", ...devices.map(item => item.id)]} labels={["不使用设备", ...devices.map(item => `${item.name || item.hostname || item.id}${item.online ? "" : "（离线）"}`)]} value={config.connector_device_id} onChange={(value: string) => setConfig(current => ({ ...current, connector_device_id: value, connector_workspace_path: value === current.connector_device_id ? current.connector_workspace_path : "" }))} /><Input colors={colors} label="设备工作目录" value={config.connector_workspace_path} onChangeText={(value: string) => setConfig(current => ({ ...current, connector_workspace_path: value }))} /><Choice colors={colors} label="设备审批" values={["manual", "safe", "full_access"]} labels={["每次确认", "安全命令自动同意", "完全自动同意"]} value={config.connector_approval_mode} onChange={(value: "manual" | "safe" | "full_access") => setConfig(current => ({ ...current, connector_approval_mode: value }))} /><Input colors={colors} label="最大 Token（0 为默认）" value={config.max_tokens} keyboardType="numeric" onChangeText={(value: string) => setConfig(current => ({ ...current, max_tokens: value }))} /><Input colors={colors} label="温度（留空为默认）" value={config.temperature} keyboardType="decimal-pad" onChangeText={(value: string) => setConfig(current => ({ ...current, temperature: value }))} /><Choice colors={colors} label="推理强度" values={["", "minimal", "low", "medium", "high"]} labels={["默认", "最小", "低", "中", "高"]} value={config.reasoning_effort} onChange={(value: string) => setConfig(current => ({ ...current, reasoning_effort: value }))} /><Toggle colors={colors} label="自动压缩上下文" value={config.auto_compress_context} onChange={(value: boolean) => setConfig(current => ({ ...current, auto_compress_context: value }))} /><View style={styles.toolSection}><Text style={{ color: colors.muted, fontSize: 12 }}>可用工具</Text>{toolGroups.map(tool => <Pressable key={tool.key} onPress={() => setConfig(current => ({ ...current, disabled_tool_groups: current.disabled_tool_groups.includes(tool.key) ? current.disabled_tool_groups.filter(item => item !== tool.key) : [...current.disabled_tool_groups, tool.key] }))} style={[styles.toolRow, { borderColor: colors.border }]}><Text style={{ color: colors.text }}>{tool.label}</Text><Ionicons name={config.disabled_tool_groups.includes(tool.key) ? "close-circle-outline" : "checkmark-circle-outline"} size={21} color={config.disabled_tool_groups.includes(tool.key) ? colors.muted : colors.text} /></Pressable>)}</View><Text style={{ color: colors.muted, fontSize: 12 }}>{activeID ? "保存后会应用于当前会话的后续消息。" : "新建会话的设置会在发送第一条消息时生效。"}</Text><Action colors={colors} title={activeID ? "保存会话设置" : "完成"} onPress={saveSessionSettings} /></ScrollView></Sheet></Modal><Modal visible={devicePickerOpen} transparent animationType="slide" onRequestClose={() => setDevicePickerOpen(false)}><Sheet colors={colors} title="选择会话设备" onClose={() => setDevicePickerOpen(false)}><ScrollView contentContainerStyle={styles.deviceList}><Pressable onPress={() => { setConfig(current => ({ ...current, connector_device_id: "", connector_workspace_path: "" })); setDevicePickerOpen(false) }} style={[styles.deviceRow, { borderColor: colors.border }]}><Ionicons name="close-circle-outline" size={21} color={colors.text} /><Text style={{ color: colors.text, flex: 1 }}>不使用设备</Text></Pressable>{devices.map(item => <Pressable key={item.id} onPress={() => { setConfig(current => ({ ...current, connector_device_id: item.id, connector_workspace_path: "" })); setDevicePickerOpen(false) }} style={[styles.deviceRow, { borderColor: colors.border }]}><Ionicons name="laptop-outline" size={21} color={colors.text} /><View style={{ flex: 1 }}><Text style={{ color: colors.text }}>{item.name || item.hostname || item.id}</Text><Text style={{ color: colors.muted, fontSize: 12 }}>{item.online ? "在线" : "离线"}</Text></View>{config.connector_device_id === item.id ? <Ionicons name="checkmark" size={20} color={colors.text} /> : null}</Pressable>)}</ScrollView></Sheet></Modal>
  </>
}
...
function configFromSession(session: Session, agent: Agent): SessionConfig { return { model_name: session.model_name || agent.default_model || "", user_channel_id: String(session.user_channel_id || 0), max_tokens: String(session.max_tokens || 0), temperature: session.temperature == null ? "" : String(session.temperature), reasoning_effort: session.reasoning_effort || "", auto_compress_context: session.auto_compress_context !== false, connector_device_id: session.connector_device_id || "", connector_workspace_path: session.connector_workspace_path || "", connector_approval_mode: session.connector_approval_mode || (session.connector_auto_approve ? "full_access" : "manual"), disabled_tool_groups: session.disabled_tool_groups || [] } }
function settingsPayload(config: SessionConfig) { return { model_name: config.model_name, user_channel_id: Number(config.user_channel_id) || 0, max_tokens: Number(config.max_tokens) || 0, temperature: config.temperature === "" ? null : Number(config.temperature), reasoning_effort: config.reasoning_effort, auto_compress_context: config.auto_compress_context, connector_device_id: config.connector_device_id || undefined, connector_workspace_path: config.connector_workspace_path || undefined, connector_approval_mode: config.connector_approval_mode, connector_auto_approve: config.connector_approval_mode === "full_access", disabled_tool_groups: config.disabled_tool_groups } }
...
const styles = StyleSheet.create({ flex: { flex: 1 }, header: { height: 58, borderBottomWidth: StyleSheet.hairlineWidth, paddingHorizontal: 10, flexDirection: "row", alignItems: "center" }, headerCenter: { flex: 1, alignItems: "center", paddingHorizontal: 4 }, headerTitle: { fontSize: 17, fontWeight: "600" }, headerActions: { flexDirection: "row", alignItems: "center" }, headerAction: { width: 34, height: 40, alignItems: "center", justifyContent: "center" }, messages: { padding: 16, gap: 10, paddingBottom: 20, flexGrow: 1 }, bubbleWrap: { alignSelf: "flex-start", maxWidth: "88%" }, userBubble: { alignSelf: "flex-end" }, bubble: { borderRadius: 16, padding: 12 }, welcome: { alignItems: "center", justifyContent: "center", flexGrow: 1, gap: 12, padding: 24 }, welcomeTitle: { fontSize: 24, fontWeight: "700" }, suggestion: { width: "100%", borderWidth: 1, borderRadius: 12, padding: 14 }, actionMenu: { borderTopWidth: StyleSheet.hairlineWidth, paddingHorizontal: 12, paddingVertical: 6, flexDirection: "row", gap: 4 }, menuAction: { flex: 1, minHeight: 56, alignItems: "center", justifyContent: "center", gap: 5 }, composer: { borderTopWidth: StyleSheet.hairlineWidth, padding: 12, flexDirection: "row", alignItems: "flex-end", gap: 10 }, input: { flex: 1, minHeight: 44, maxHeight: 130, borderRadius: 14, paddingHorizontal: 12, paddingVertical: 10, fontSize: 16 }, attachments: { paddingHorizontal: 12, paddingVertical: 6, gap: 8 }, attachment: { maxWidth: 220, height: 32, borderRadius: 9, paddingHorizontal: 8, flexDirection: "row", alignItems: "center", gap: 6 }, drawerOverlay: { flex: 1, flexDirection: "row", backgroundColor: "rgba(0,0,0,.3)" }, drawerSpace: { flex: 1 }, drawer: { width: "82%", maxWidth: 360, padding: 18 }, drawerTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }, drawerTitle: { fontSize: 22, fontWeight: "700" }, newButton: { height: 46, paddingHorizontal: 12, borderRadius: 10, flexDirection: "row", alignItems: "center", gap: 7, marginBottom: 8 }, session: { padding: 12, borderRadius: 10 }, settingsForm: { gap: 13, paddingBottom: 16 }, toolSection: { gap: 0 }, toolRow: { minHeight: 46, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, deviceList: { gap: 0, paddingBottom: 16 }, deviceRow: { minHeight: 62, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: 10 }, pressed: { opacity: .58, transform: [{ scale: .985 }] } })
