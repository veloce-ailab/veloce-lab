...
function BottomTabs({ colors, active, onChange, bottom }: { colors: Palette; active: Tab; onChange: (tab: Tab) => void; bottom: number }) {
  ...
  return <View style={[styles.tabs, { borderColor: colors.border, backgroundColor: colors.card, paddingBottom: bottom }]}><View style={styles.tabContent}>{tabs.map(item => <Pressable key={item.key} onPress={() => onChange(item.key)} android_ripple={{ color: colors.input }} style={({ pressed }) => [styles.tab, active === item.key && { backgroundColor: colors.primary }, pressed && styles.pressed]}><Ionicons name={item.icon} size={21} color={active === item.key ? colors.primaryText : colors.muted} /><Text style={[styles.tabLabel, { color: active === item.key ? colors.primaryText : colors.muted }]}>{item.label}</Text></Pressable>)}</View></View>
}

const styles = StyleSheet.create({ app: { flex: 1 }, safe: { flex: 1 }, content: { flex: 1, width: "100%", maxWidth: 760, alignSelf: "center" }, loading: { flex: 1, alignItems: "center", justifyContent: "center" }, tabs: { minHeight: 65, borderTopWidth: StyleSheet.hairlineWidth }, tabContent: { width: "100%", maxWidth: 760, alignSelf: "center", flexDirection: "row", justifyContent: "space-around", paddingHorizontal: 6, paddingTop: 5 }, tab: { flex: 1, minHeight: 48, borderRadius: 11, alignItems: "center", justifyContent: "center", gap: 2 }, pressed: { opacity: .55, transform: [{ scale: .96 }] }, tabLabel: { fontSize: 11, fontWeight: "500" } })
