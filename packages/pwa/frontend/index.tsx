...
function PWARegistration({ themeColor, iconUrl, windowControlsOverlay = true }: { themeColor?: string; iconUrl?: string; windowControlsOverlay?: boolean }) {
  useEffect(() => {
    ...
    const overlayStyle = document.createElement("style"); overlayStyle.textContent = ".pwa-window-controls-overlay .pwa-titlebar{width:env(titlebar-area-width,100%);height:env(titlebar-area-height,auto);margin-left:env(titlebar-area-x,0px);-webkit-app-region:drag}.pwa-window-controls-overlay .pwa-titlebar a,.pwa-window-controls-overlay .pwa-titlebar button,.pwa-window-controls-overlay .pwa-titlebar input,.pwa-window-controls-overlay .pwa-titlebar select{-webkit-app-region:no-drag}"; document.head.append(overlayStyle)
  ...
  }, [themeColor, iconUrl, windowControlsOverlay])
  return null
}
