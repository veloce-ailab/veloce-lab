...
export interface DashboardService { registerSlot(slot: DashboardSlot): () => void; slots(): DashboardSlot[]; registerAsset(asset: DashboardAsset): () => void; assets(): DashboardAsset[]; addEntry(entry: DashboardEntry): () => void; }
...
export function apply(ctx: Context, cfg: { enabled: boolean }) {
  ...
  const service: DashboardService = { registerSlot(slot) { slots.push(slot); return () => { const i = slots.indexOf(slot); if (i >= 0) slots.splice(i, 1); }; }, slots: () => [...slots].sort((a, b) => (a.order ?? 0) - (b.order ?? 0)), registerAsset(asset) { const value = { ...asset, id: asset.id || createHash("md5").update(asset.file).digest("hex") }; assets.push(value); return () => { const i = assets.indexOf(value); if (i >= 0) assets.splice(i, 1); }; }, assets: () => [...assets], addEntry(entry) { entries.push(entry); const file = entry.prod; const asset = { id: createHash("md5").update(file).digest("hex"), file, plugin: entry.plugin, mime: "text/javascript; charset=utf-8" }; const remove = service.registerAsset(asset); return () => { const i = entries.indexOf(entry); if (i >= 0) entries.splice(i, 1); remove(); }; } };
