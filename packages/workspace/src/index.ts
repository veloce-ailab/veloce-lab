...
export function apply(ctx: Context, cfg: { enabled: boolean }) {
  ...
  ctx.route("/api/user/advanced-chat/files").methods("GET").action(async s => { const id = user(s); if (id) s.respond(await db.select("advanced_chat_files", { user_id: id }), "json"); });
  ctx.route("/api/user/advanced-chat/files").methods("POST").action(async s => { const id = user(s); if (!id) return; const input = await s.parseRequestBody() as any; const data = String(input.data ?? input.content ?? ""); const now = new Date().toISOString(); const row = await db.create("advanced_chat_files", { id: randomUUID(), user_id: id, name: String(input.name ?? "file"), mime_type: String(input.mime_type ?? "text/plain"), size: Buffer.byteLength(data), data, storage_path: "", text_extract: data, hash: "", source: "upload", source_key: "", created_at: now, updated_at: now } as any); s.status = 201; s.respond(row, "json"); });
  ctx.route("/api/user/advanced-chat/files/:id/content").methods("GET").action(async (s, _p, fileId) => { const id = user(s); const row = id ? await db.selectOne("advanced_chat_files", { id: fileId, user_id: id }) : undefined; if (!row) { s.status = 404; s.respond({ error: "File not found" }, "json"); return; } s.respond({ content: row.data ?? row.text_extract ?? "" }, "json"); });
  ctx.route("/api/user/advanced-chat/files/:id").methods("DELETE").action(async (s, _p, fileId) => { const id = user(s); if (id) { await db.remove("advanced_chat_files", { id: fileId, user_id: id }); s.respond({ success: true }, "json"); } });
}
