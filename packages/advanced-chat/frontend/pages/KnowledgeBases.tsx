...
import { ArrowLeft, Download, FileText, FolderOpen, Pencil, Plus, RefreshCw, Trash2, Upload } from "lucide-react"
...
function normalizeKnowledgeBase(value: unknown): KnowledgeBase | null { if (!isRecord(value) || typeof value.id !== "string" || !value.id) return null; return { id: value.id, name: typeof value.name === "string" ? value.name : value.id, description: typeof value.description === "string" ? value.description : "", document_count: Number(value.document_count || 0), embedding_model_name: typeof value.embedding_model_name === "string" ? value.embedding_model_name : "", embedding_user_channel_id: Number(value.embedding_user_channel_id || 0), vectorized: value.vectorized === true, created_at: typeof value.created_at === "string" ? value.created_at : "", updated_at: typeof value.updated_at === "string" ? value.updated_at : "" } }
function normalizeKnowledgeDocuments(value: unknown): KnowledgeDocumentsResponse { const item = isRecord(value) ? value : {}; const documents = Array.isArray(item.documents) ? item.documents.map(normalizeKnowledgeDocument).filter((document): document is KnowledgeDocument => Boolean(document)) : []; return { documents } }
