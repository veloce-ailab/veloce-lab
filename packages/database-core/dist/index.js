...
export class SqlDatabase {
    ...
    async extend(table, schema, indexes) {
        ...
        for (const [name, input] of Object.entries(schema)) {
            ...
            try {
                await this.driver.execute(`ALTER TABLE ${identifier(String(table))} ADD COLUMN ${column}`);
            }
            catch {
    ...
    async remove(table, query) {
        const params = [];
        const condition = where(query, params);
        const result = await this.driver.execute(`DELETE FROM ${identifier(String(table))}${condition ? ` WHERE ${condition}` : ""}`, params);
        return result.changes ?? 0;
    }
    async upsert(table, data, key, update) {
        for (const row of data) {
            const existing = await this.selectOne(table, Object.fromEntries((Array.isArray(key) ? key : [key]).map((field) => [
                field,
                row[field],
            ])));
            if (existing)
                await this.update(table, Object.fromEntries((Array.isArray(key) ? key : [key]).map((field) => [
                    field,
                    row[field],
                ])), update ?? row);
            else
    ...
    async drop(table) {
        await this.driver.execute(`DROP TABLE IF EXISTS ${identifier(String(table))}`);
    }
    run(sql, params) {
