...
export async function apply(ctx: Context) {
  ...
  ctx.route("/api/user/advanced-chat/subagents").methods("GET").action(async (session) => { const current = owner(session); if (current?.id === undefined) return; const sessionId = String(session.query.session_id ?? ""); const rows = await db.select("advanced_chat_subagent_tasks", { user_id: current.id, ...(sessionId ? { session_id: sessionId } : {}) }); session.respond(rows, "json"); });
